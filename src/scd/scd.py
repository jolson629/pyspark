...
def main(argv):
   ...
   new_state_raw_schema = StructType([
      StructField("new_user_id", IntegerType(), True),
      StructField("new_address", StringType(), True)
   ])
   ...
   df_new_state = df_new_state_raw \
      .withColumn('new_active_date', lit(current_time)) \
      .withColumn('new_inactive_date', lit(high_time))


   # Full outer join: join on key column and also inactive time column to make only join to the latest records
   df_merge = df_current_state.join(df_new_state, (df_new_state.new_user_id == df_current_state.user_id) & (df_new_state.new_inactive_date == df_current_state.inactive_date), how='fullouter')

   # Derive new column to indicate the action
   df_merge = df_merge.withColumn('action',
      when(df_merge.address != df_merge.new_address, 'UPSERT')
         .when(df_merge.new_user_id.isNull() & df_merge.is_current, 'DELETE')
         .when(df_merge.user_id.isNull(), 'INSERT')
         .otherwise('NOACTION')
   )
   ...
   column_names = ['user_id', 'address', 'is_current', 'is_deleted', 'active_date', 'inactive_date']
   ...
   df_insert = df_merge.filter(df_merge.action == 'INSERT').select(
      df_merge.new_user_id.alias('user_id'),
      df_merge.new_address.alias('address'),
         lit(True).alias('is_current'),
         lit(False).alias('is_deleted'),
         df_merge.new_active_date.alias('active_date'), 
         df_merge.new_inactive_date.alias('inactive_date')
   )
   
   # For records that needs to be deleted
   df_delete = df_merge.filter(df_merge.action == 'DELETE').select(
      df_merge.user_id.alias('user_id'),
      df_merge.address.alias('address'),
   ...
   df_upsert_expired = df_merge \
      ...
      .withColumn('inactive_date', df_merge.new_active_date).withColumn('is_current', lit(False)) \
   ...
   df_merge_final.orderBy(['user_id', 'active_date']).show(df_merge_final.count(), False)
